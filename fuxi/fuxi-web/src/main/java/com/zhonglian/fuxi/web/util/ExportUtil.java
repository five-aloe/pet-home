package com.zhonglian.fuxi.web.util;
...
public class ExportUtil {
	 ...
	 * @param response 返回结果
	 */
	public static boolean doExport(JSONArray dataList, String colNames, String mapKey, OutputStream os) {
		try {
			StringBuffer buf = new StringBuffer();
			...
			if (null != dataList) { // 输出数据
				for (int i = 0; i < dataList.length(); i++) {
					for (int j = 0; j < mapKeyArr.length; j++) {
						buf.append(dataList.getJSONObject(i).getString(mapKeyArr[j])).append(CSV_COLUMN_SEPARATOR);
					}
					buf.append(CSV_RN);
				}
			}
	 ...
	 *             setHeader
	 */
	public static void responseSetPropertiesNotSuffix(String fileName, HttpServletResponse response) throws UnsupportedEncodingException {
		// 设置文件后缀
