...
public class UserServiceimpl implements UserService {
	...
	private void updateAcount(List<Account> acountList, List<AccountRoleEnums> companyRole, String mobile,
		...
		if (companyRole.size() > 0) {
			for(AccountRoleEnums acountRole :companyRole){
				Account acounntRole = accountServiceApi.findByUserRole(userId, acountRole);
				if(acounntRole != null){
					//资金账户复用
					accountServiceApi.updateByUserRole(acounntRole.getNo());
				}else{
					// 开通资金账户
